...
func newMysqlDbCurrConnections() *prometheus.GaugeVec {
	return prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Namespace: "oci_exporter",
			Name:      "mysql_db_current_connections",
			Help:      "Current Connections of OCI MySQL Database.",
		},
		[]string{
			"compartment_id",
			"resource_name",
...
func GetMysqlDbCurrConnections(ctx context.Context, mysqlDbCurrConnections *prometheus.GaugeVec) error {
	mysqlDbCurrConnections.Reset()
	...
	err := getMysqlDbCurrConnectionsByCompartment(
		ctx,
		compartmentId,
		query,
		namespaceQuery,
...
func getMysqlDbCurrConnectionsByCompartment(
	ctx context.Context,
	compartmentId string,
	query string,
	namespaceQuery string,
	mysqlDbCurrConnections *prometheus.GaugeVec,
) error {
	...
	for _, metric := range resp.Items {
		...
		// set gauge value
		mysqlDbCurrConnections.With(prometheus.Labels{
